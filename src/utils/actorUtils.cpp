	...
	void InflictSizeDamage(Actor* attacker, Actor* receiver, float value) {
		if (!receiver->IsDead()) {
			...
			if (receiver->GetCharController()) {
				...
				log::info("Flag 0: {}", controller.flags.GetValues(CHARACTER_FLAGS::kNone));
				log::info("Flag 1: {}", controller.flags.GetValues(CHARACTER_FLAGS::kQuadruped));
				log::info("Flag 2: {}", controller.flags.GetValues(CHARACTER_FLAGS::kNoGravityOnGround));
				log::info("Flag 3: {}", controller.flags.GetValues(CHARACTER_FLAGS::kTryStep));
				log::info("Flag 4: {}", controller.flags.GetValues(CHARACTER_FLAGS::kNoFriction));
				log::info("Flag 5: {}", controller.flags.GetValues(CHARACTER_FLAGS::kAllowJumpNoContact));
				log::info("Flag 6: {}", controller.flags.GetValues(CHARACTER_FLAGS::kStuckQuad));
				log::info("Flag 7: {}", controller.flags.GetValues(CHARACTER_FLAGS::kAnimAngleMod));
				log::info("Flag 8: {}", controller.flags.GetValues(CHARACTER_FLAGS::kHitDamage));
				log::info("Flag 9: {}", controller.flags.GetValues(CHARACTER_FLAGS::kHitFlags));
				log::info("Flag 10: {}", controller.flags.GetValues(CHARACTER_FLAGS::kSupport));
				log::info("Flag 11: {}", controller.flags.GetValues(CHARACTER_FLAGS::kHasPotentialSupportManifold));
				log::info("Flag 12: {}", controller.flags.GetValues(CHARACTER_FLAGS::kCanJump));
				log::info("Flag 13: {}", controller.flags.GetValues(CHARACTER_FLAGS::kChaseBip));
				log::info("Flag 14: {}", controller.flags.GetValues(CHARACTER_FLAGS::kFollowRagdoll));
				log::info("Flag 15: {}", controller.flags.GetValues(CHARACTER_FLAGS::kJumping));
				log::info("Flag 16: {}", controller.flags.GetValues(CHARACTER_FLAGS::kNotPushable));
				log::info("Flag 17: {}", controller.flags.GetValues(CHARACTER_FLAGS::kFloatLand));
				log::info("Flag 18: {}", controller.flags.GetValues(CHARACTER_FLAGS::kCheckSupport));
				log::info("Flag 19: {}", controller.flags.GetValues(CHARACTER_FLAGS::kNoSim));
				log::info("Flag 20: {}", controller.flags.GetValues(CHARACTER_FLAGS::kFarAway));
				log::info("Flag 21: {}", controller.flags.GetValues(CHARACTER_FLAGS::kOnStilts));
				log::info("Flag 22: {}", controller.flags.GetValues(CHARACTER_FLAGS::kQuickSimulate));
				log::info("Flag 23: {}", controller.flags.GetValues(CHARACTER_FLAGS::kRecordHits));
				log::info("Flag 24: {}", controller.flags.GetValues(CHARACTER_FLAGS::kComputeTiltPreIntegrate));
				log::info("Flag 25: {}", controller.flags.GetValues(CHARACTER_FLAGS::kShouldersUnderWater));
				log::info("Flag 26: {}", controller.flags.GetValues(CHARACTER_FLAGS::kOnStairs));
				log::info("Flag 27: {}", controller.flags.GetValues(CHARACTER_FLAGS::kCanPitch));
				log::info("Flag 28: {}", controller.flags.GetValues(CHARACTER_FLAGS::kCanRoll));
				log::info("Flag 29: {}", controller.flags.GetValues(CHARACTER_FLAGS::kNoCharacterCollisions));
				log::info("Flag 30: {}", controller.flags.GetValues(CHARACTER_FLAGS::kNotPushablePermanent));
				log::info("Flag 31: {}", controller.flags.GetValues(CHARACTER_FLAGS::kPossiblePathObstacle));
				log::info("Flag 32: {}", controller.flags.GetValues(CHARACTER_FLAGS::kShapeRequiresZRot));
				log::info("Flag 33: {}", controller.flags.GetValues(CHARACTER_FLAGS::kSwimAtWaterSurface));
