...
namespace Gts {
 ...
 void SwordOfSize(Actor* caster, Actor* target)
  ...
  void ManageGameModePC()
  {
    ...
    {mod_target_scale(Player, (0.00015 - (ShrinkRate * 0.25) * Scale));}
    else if (runtime.ChosenGameMode->value == 3.0 && Scale < size_limit)
    { if (Player->IsInCombat() == true)
      {mod_target_scale(Player, (0.00008 + (GrowthRate * 0.17) * Scale));}
      else if (Player->IsInCombat() == false)
      {mod_target_scale(Player, - (0.00010 + (ShrinkRate * 0.34) * Scale));}
  }
  }
    ...
    void ManageGameModeNPC(Actor* Receiver)
  {
    ...
    {mod_target_scale(Receiver, (0.00015 - (ShrinkRate * 0.25) * Scale));}
    else if (runtime.ChosenGameModeNPC->value == 3.0 && Scale < size_limit)
    { if (Receiver->IsInCombat() == true)
      {mod_target_scale(Receiver, (0.00008 + (GrowthRate * 0.17) * Scale));}
      else if (Receiver->IsInCombat() == false)
      {mod_target_scale(Receiver,-(0.00010 + (ShrinkRate * 0.34) * Scale));}
  }
}
