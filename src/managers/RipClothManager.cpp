...
namespace Gts {
	...
	std::vector<TESObjectARMO*> ParseArmors(RE::Actor *Act) {
        ...
        for (auto Slot : VallidSlots) {
			...
			for (const auto &BKwd : KeywordBlackList) {
                if (Armor->HasKeywordString(BKwd)) continue;	//If blacklisted keyword is found skip
    ...
    void DoRip(RE::Actor* Act, RE::TESObjectARMO* Slot) {
        ...
        Rumble::Once("ClothManager", Act, (32 * get_visual_scale(Act)), 0.05);
    }
	...
	void ClothManager::CheckRip() {
		...
		if (ArmorCount == 0) return; //No Vallid Armors Found
