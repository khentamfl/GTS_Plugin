...
namespace {
	...
	void AdjustGiantessSkill(Actor* Caster, Actor* Target) { // Adjust Matter Of Size skill on Crush
		...
		float Total = (((0.18 * random) + absorbedSize/40) * ValueEffectiveness);
			...
			if (data.state == CrushState::Healthy) {
				...
				if (data.delay.ShouldRun()) {
					...
					if (!IsLiving(tiny)) {
						...
						if (root) {
							SpawnParticle(tiny, 0.20, "GTS/Damage/FootExplosion.nif", NiMatrix3(), root->world.translate, currentSize * 12, 7, root);
						}
					} else {
						auto root = find_node(tiny, "NPC Root [Root]");
						if (root) {
