...
namespace {
    ...
    void ApplyDamageOverTime_Left(Actor* giant) {
		...
		TaskManager::Run(name, [=](auto& progressData) {
			...
			DoDamageEffect(giantref, 0.008, 1.5, 10000, 0.05, FootEvent::Left, 2.5, SizeDamageSource::FootGrinded);
			return true;
		});
	}
     ...
     void ApplyDamageOverTime_Right(Actor* giant) {
		...
		TaskManager::Run(name, [=](auto& progressData) {
			...
			DoDamageEffect(giantref, 0.008, 1.5, 10000, 0.05, FootEvent::Right, 2.5, SizeDamageSource::FootGrinded);
			return true;
		});
	}
    ...
    void GTSstomp_FootGrindR_Impact(AnimationEventData& data) { // When foot hits the ground after lifting the leg up. R Foot
        ...
        DoDamageEffect(&data.giant, 2.0, 1.70, 20, 0.15, FootEvent::Right, 1.0, SizeDamageSource::FootGrinded);
    ...
    void GTSstomp_FootGrindL_Impact(AnimationEventData& data) { // When foot hits the ground after lifting the leg up. L Foot
        ...
        DoDamageEffect(&data.giant, 2.0, 1.70, 20, 0.15, FootEvent::Left, 1.0, SizeDamageSource::FootGrinded);
