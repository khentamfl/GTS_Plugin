...
namespace {
	...
	void GTSSandwich_FootImpact(AnimationEventData& data) {
		...
		DoDamageEffect(&data.giant, 4.0 * perk, 2.6, 10, 0.10, FootEvent::Right);
		DoDamageEffect(&data.giant, 4.0 * perk, 2.6, 10, 0.10, FootEvent::Left);
		DoLaunch(&data.giant, 0.85 * perk, 2.0, RNode, 1.0);
		DoLaunch(&data.giant, 0.85 * perk, 2.0, LNode, 1.0);
	}
