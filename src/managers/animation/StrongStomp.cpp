...
namespace {
	...
	void GTS_StrongStomp_ImpactR(AnimationEventData& data) {
		...
		DoDamageEffect(&data.giant, damage * (4.8 + data.animSpeed/2) * perk, (1.80 + data.animSpeed/4) * damage, 5, 0.035, FootEvent::Right, 1.0);
		DoSizeEffect(&data.giant, SMT + (data.animSpeed * 0.05), FootEvent::Right, RNode); 
		DoLaunch(&data.giant, 0.95 * perk, 7.0 + data.animSpeed/2, 2.0, FootEvent::Right, 1.0);
	...
	void GTS_StrongStomp_ImpactL(AnimationEventData& data) {
		...
		DoDamageEffect(&data.giant, damage * (4.8 + data.animSpeed/2) * perk, (1.80 + data.animSpeed/4) * damage, 5, 0.035, FootEvent::Left, 1.0);
		DoSizeEffect(&data.giant, SMT + (data.animSpeed * 0.05), FootEvent::Left, LNode); 
		DoLaunch(&data.giant, 0.95 * perk, 7.0 + data.animSpeed/2, 2.0, FootEvent::Left, 1.0);
		DrainStamina(&data.giant, "StaminaDrain_StrongStomp", "DestructionBasics", false, 1.45, 2.8);
		data.canEditAnimSpeed = false;
		data.animSpeed = 1.0;
	}
