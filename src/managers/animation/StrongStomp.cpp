...
namespace {
	...
	void GTS_StrongStomp_ImpactR(AnimationEventData& data) {
		...
		DoDamageEffect(&data.giant, damage * (4.8 + data.animSpeed/2) * perk, (1.80 + data.animSpeed/4) * damage, 5, 0.35, FootEvent::Right, 1.0, SizeDamageSource::Crushed);
		...
		DoLaunch(&data.giant, 0.95 * perk, 7.0 + data.animSpeed/2, 1.0, FootEvent::Right, 1.15);
	...
	void GTS_StrongStomp_ImpactL(AnimationEventData& data) {
		...
		DoDamageEffect(&data.giant, damage * (4.8 + data.animSpeed/2) * perk, (1.80 + data.animSpeed/4) * damage, 5, 0.35, FootEvent::Left, 1.0, SizeDamageSource::Crushed);
		...
		DoLaunch(&data.giant, 0.95 * perk, 7.0 + data.animSpeed/2, 1.0, FootEvent::Left, 1.15);
		DrainStamina(&data.giant, "StaminaDrain_StrongStomp", "DestructionBasics", false, 1.45, 2.8);
		data.stage = 0;
		data.canEditAnimSpeed = false;
		data.animSpeed = 1.0;
