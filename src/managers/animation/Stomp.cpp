...
namespace {
	...
	void GTSstompimpactR(AnimationEventData& data) {
		...
		DoDamageEffect(&data.giant, (1.8 + data.animSpeed/8) * launch * perk, (1.45 + data.animSpeed/4) * launch, 10, 0.25, FootEvent::Right, 1.0, DeathCause::Crushed);
	...
	void GTSstompimpactL(AnimationEventData& data) { // To-do: Add Foot Grind support  (Trigger it somehow)
		...
		DoDamageEffect(&data.giant, (1.8 + data.animSpeed/8) * launch * perk, (1.45 + data.animSpeed/4) * launch, 10, 0.25, FootEvent::Left, 1.0, DeathCause::Crushed);
	...
	void GTSstomplandR(AnimationEventData& data) { // To-do: Add Foot Grind support (Trigger it somehow)
		...
		DoDamageEffect(&data.giant, 1.6 * perk, 1.45, 25, 0.25, FootEvent::Right, 1.0, DeathCause::Crushed);
	...
	void GTSstomplandL(AnimationEventData& data) {
		...
		DoDamageEffect(&data.giant, 1.6 * perk, 1.45, 25, 0.25, FootEvent::Left, 1.0, DeathCause::Crushed);
		DoFootstepSound(&data.giant, 1.0 + data.animSpeed/14, FootEvent::Left, LNode);
		DoDustExplosion(&data.giant, dust + data.animSpeed/4, FootEvent::Left, LNode);
