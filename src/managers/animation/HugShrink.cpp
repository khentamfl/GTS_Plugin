...
namespace Gts {
	...
	void HugShrink::AttachActorTask(Actor* giant, Actor* tiny) {
		...
		TaskManager::Run(name, [=](auto& progressData) {
			...
			tiny->data.angle = (giant->data.angle.z + math.M_PI) % (2 * math.M_PI);

			float TinyDegree = tiny->data.angle.z * 180/math.M_PI;
			float GiantDegree = giant->data.angle.z * 180/math.M_PI;
