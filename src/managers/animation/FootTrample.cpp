...
namespace {
	...
	void GTS_Trample_Footstep_L(AnimationEventData& data) { // Stage 1 footsteps
		...
		DoDamageEffect(&data.giant, 0.7 * launch * perk, 1.50 * launch, 100, 0.10, FootEvent::Left, 1.10, DamageSource::CrushedLeft);
	...
	void GTS_Trample_Footstep_R(AnimationEventData& data) { // stage 1 footsteps
		...
		DoDamageEffect(&data.giant, 0.7 * launch * perk, 1.50 * launch, 100, 0.10, FootEvent::Right, 1.10, DamageSource::CrushedRight);
	...
	void GTS_Trample_Impact_L(AnimationEventData& data) { // Stage 2 repeating footsteps
		...
		DoDamageEffect(&data.giant, 1.2 * launch * perk, 1.85 * launch, 1, 0.12, FootEvent::Left, 1.10, DamageSource::CrushedLeft);
		DoFootstepSound(&data.giant, 1.0, FootEvent::Left, LNode);
		DoDustExplosion(&data.giant, dust, FootEvent::Left, LNode);
	...
	void GTS_Trample_Impact_R(AnimationEventData& data) { // Stage 2 repeating footsteps
		...
		DoDamageEffect(&data.giant, 1.2 * launch * perk, 1.85 * launch, 1, 0.12, FootEvent::Right, 1.10, DamageSource::CrushedRight);
		DoFootstepSound(&data.giant, 1.0, FootEvent::Right, RNode);
		DoDustExplosion(&data.giant, dust, FootEvent::Right, RNode);
