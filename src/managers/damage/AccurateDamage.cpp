		...
		if (persistent.GetData(Caster)) {
			if (persistent.GetData(Caster)->smt_run_speed >= 1.0) {
				...
				if (CasterHp >= (TargetHp / Multiplier) && !CrushManager::AlreadyCrushed(Target)) {
					...
					CrushBonuses(Caster, Target, DeathCause::Collision);
...
namespace Gts {
	...
	void AccurateDamage::DoAccurateCollisionLeft(Actor* actor, float damage, float radius, int random, float bbmult, float crushmult, DeathCause Cause) { // Called from GtsManager.cpp, checks if someone is close enough, then calls DoSizeDamage()
	...
	void AccurateDamage::DoAccurateCollisionRight(Actor* actor, float damage, float radius, int random, float bbmult, float crushmult, DeathCause Cause) { // Called from GtsManager.cpp, checks if someone is close enough, then calls DoSizeDamage()
	...
	void AccurateDamage::ApplySizeEffect(Actor* giant, Actor* tiny, float force, int random, float bbmult, float crushmult, DeathCause Cause) {
	...
	void AccurateDamage::DoSizeDamage(Actor* giant, Actor* tiny, float totaldamage, float mult, int random, float bbmult, bool DoDamage, float crushmult, DeathCause Cause) { // Applies damage and crushing
