	...
	void apply_speed(Actor* actor, ActorData* persi_actor_data, TempActorData* trans_actor_data, bool force = false) {
		...
		float NegativeMS = clamp(1.0, 0.44, 1.05 - scale/20); // Used to slow down SlowWalk speed at huge size
			...
			if (actor->IsSprinting() == true) {
				...
				log::info("Normal Adjusting MS of {}, BaseWS: {}, Ms_Mult: {}, kSpeedMult: {}, Bonus: {}, NegativeMS: {}", actor->GetDisplayFullName(), trans_actor_data->base_walkspeedmult, MS_mult, MovementSpeed, BonusSprint, NegativeMS);
			}
		}
