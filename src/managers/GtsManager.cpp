...
void GtsManager::Update() {
	...
	for (auto actor: find_actors()) {
		...
		if (actor->formID == 0x14 || IsTeammate(actor)) {
			accuratedamage.DoAccurateCollisionLeft(actor, 0.4, 1.0, 2000, 0.05, 2.5, DamageSource::CrushedLeft);
			accuratedamage.DoAccurateCollisionRight(actor, 0.4, 1.0, 2000, 0.05, 2.5, DamageSource::CrushedRight);
		...
		if (Runtime::GetBool("PreciseDamageOthers")) {
			if (actor->formID != 0x14 && !actor->IsPlayerTeammate() && !Runtime::InFaction(actor, "FollowerFaction")) {
				accuratedamage.DoAccurateCollisionLeft(actor, 0.4, 1.0, 1000, 0.25, 2.5, DamageSource::CrushedLeft);
				accuratedamage.DoAccurateCollisionRight(actor, 0.4, 1.0, 1000, 0.25, 2.5, DamageSource::CrushedRight);
			}
		}
