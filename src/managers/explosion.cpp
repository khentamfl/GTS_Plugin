...
namespace {
	void make_explosion_at(FootEvent kind, Actor* actor, NiPoint3 position, float scale) {
		...
		switch (kind) {
			case FootEvent::Left:
				if (LF) {
					SpawnParticle(actor, 3.0, "GTS/FootExplosion.nif", NiMatrix3(), position, scale, 7, LF);
				}
			case FootEvent::Right:
				if (RF) {
					SpawnParticle(actor, 3.0, "GTS/FootExplosion.nif", NiMatrix3(), position, scale, 7, RF);
				}
